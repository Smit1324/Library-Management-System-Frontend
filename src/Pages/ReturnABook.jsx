...
import { returnBook } from '../actions/userActions';
...
const ReturnABook = () => {
    ...
    const returnABook = async e => {
        e.preventDefault();
        setLoader(true)

        try {

            const res = await fetch(`https://library-management-system-server.onrender.com/api/users/returnbook/${id}`, {
                method: "PUT",
                headers: {
            ...
            <div className='h-screen w-full flex items-start justify-center'>
                ...
                <div className={`w-10/12 h-4/6 px-20 flex items-center justify-between border-2 border-blue-200 rounded-xl shadow-xl shadow-blue-100 mt-16`}>
                    ...
                    <div className={`w-3/5 flex flex-col items-start justify-center text-blue-400`}>
                        ...
                        <button
                            className={loader ? `flex items-center justify-center border-2 border-blue-500 text-lg bg-blue-500 text-white w-40 h-11 rounded-lg mt-16` : `flex items-center justify-center border-2 border-blue-500 text-blue-600 text-lg hover:bg-blue-500 hover:text-white w-40 h-11 rounded-lg mt-16`}
                            onClick={loader ? e => e.preventDefault() : returnABook}
                        >
                            {loader ? <ColorRing
                                visible={true}
                                height="40"
